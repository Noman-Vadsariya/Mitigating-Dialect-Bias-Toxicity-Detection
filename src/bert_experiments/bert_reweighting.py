...
RESULTS_DIR = Path("../../data/results/bert_reweighting")
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

MAX_LEN = 128
ALPHA_VALUES = np.arange(2.25, 6.75, 0.75)  # [2.25, 3.0, 3.75, 4.5, 5.25, 6.0]
...
TRAIN_ARGS = dict(
    ...
    report_to="none",
    remove_unused_columns=False,  # needed for sample_weight
    seed=SEED,
...
class ToxicityDataset(torch.utils.data.Dataset):
    def __init__(self, df: pd.DataFrame, alpha: float = 1.0, max_len: int = 128):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Reweight AAE non-toxic samples
        item["sample_weight"] = torch.tensor(self.alpha if (dialect == 1 and label == 0) else 1.0, dtype=torch.float)
...
# TRAINER
# =========================
class ReweightingTrainer(Trainer):
...
def run_one_alpha(alpha: float, train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame):
    train_ds = ToxicityDataset(train_df, alpha=alpha, max_len=MAX_LEN)
    val_ds = ToxicityDataset(val_df, alpha=alpha, max_len=MAX_LEN)
    test_ds = ToxicityDataset(test_df, alpha=alpha, max_len=MAX_LEN)

    model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=2)

    args = TrainingArguments(**TRAIN_ARGS)
    args.output_dir = str(RESULTS_DIR / f"alpha_{alpha:.2f}".replace(".", "_"))
    ...
    pred_df["pred"] = preds
    pred_df.to_csv(RESULTS_DIR / f"test_predictions_alpha_{alpha:.2f}.csv".replace(".", "_"), index=False)
...
def main():
    ...
    for alpha in ALPHA_VALUES:
        print(f"\n=== Reweighting alpha={alpha:.2f} ===")
        metrics = run_one_alpha(float(alpha), train_df, val_df, test_df)
        metrics["alpha"] = float(alpha)
        rows.append(metrics)
        print(metrics)

    metrics_df = pd.DataFrame(rows).sort_values("alpha").reset_index(drop=True)
    metrics_df.to_csv(RESULTS_DIR / "reweighting_sweep_metrics.csv", index=False)
    print("\nSaved:", RESULTS_DIR / "reweighting_sweep_metrics.csv")
    ...
    plt.xlabel("Alpha")
    plt.ylabel("FPR")
    plt.title("FPR vs Reweighting")
    plt.legend()
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "fpr_vs_alpha.png", dpi=300)
    plt.close()
    ...
    plt.xlabel("Alpha")
    plt.ylabel("FNR")
    plt.title("FNR vs Reweighting")
    plt.legend()
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "fnr_vs_alpha.png", dpi=300)
    plt.close()

    plt.figure(figsize=(8, 5))
    plt.plot(metrics_df["alpha"], metrics_df["f1"], marker="o", label="F1")
    plt.xlabel("Alpha")
    plt.ylabel("F1")
    plt.title("F1 vs Reweighting")
    plt.legend()
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / "f1_vs_alpha.png", dpi=300)
    plt.close()
