"""
Pure Adversarial Debiasing Grid Search
=======================================

Runs a grid search over (lambda_adv, adv_c) using ONLY leaf-based
adversarial debiasing. No sample reweighting, no post-hoc threshold
tuning, no T1/T2/T3/T4 post-processing.

For each grid cell, records both validation and test metrics
(per-group FPR/FNR, gaps, F1, accuracy, DI) so results are directly
comparable to the original run and the ablation study.
"""
...
from train_adv_xgb import load_split, sigmoid, train_one_adv_model  # noqa: E402
from fairness_postprocess import compute_metrics  # noqa: E402
...
def main():
    ...
    p.add_argument("--out_dir", default="data/results/adv_xgb_largegrid")
    ...
    p.add_argument("--lambda_grid", required=True,
                   help="Comma-separated lambda_adv values")
    p.add_argument("--adv_c_grid", required=True,
                   help="Comma-separated adv_c values")
    args = p.parse_args()
    ...
    print("Config: leaf_adv=True, reweighting=FALSE, threshold_tuning=FALSE, "
          "post_processing=NONE (pure adversarial)")
    ...
    print(f"Grid: {len(cells)} cells  lambda={lambda_grid}  adv_c={adv_c_grid}")
    ...
    for i, (lam, c) in enumerate(cells, 1):
        ...
        booster, _ = train_one_adv_model(
            ...
            use_leaf_adv=True,
            use_reweighting=False,   # << no reweighting
        )
